...
//                   .ok();
            ...
            String s0="insert into db_bulletin.users(user_name,password,user_level,user_code)values(?,?,?,?)";
    ...
    public static String  get_name(String un){
        ...
        try {
            Connection conn = PoolConnection.connect();
            String s0="select user_code from db_bulletin.users where user_name='"+un+"'";
    ...
    public static String  get_id(String names){
        ...
        try {
            Connection conn = PoolConnection.connect();
            String s0="select id from db_bulletin.users where user_code='"+names+"'";
            Statement stmt = conn.createStatement();
            ResultSet rs = stmt.executeQuery(s0);
            while (rs.next()) {
                name = rs.getString(1);
