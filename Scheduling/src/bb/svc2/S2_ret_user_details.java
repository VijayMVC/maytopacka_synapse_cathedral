...
public class S2_ret_user_details {
    ...
    public static List<to_users.u> get_user_data() {
        ...
        String s0 = "select user_name,user_level,user_code from db_bulletin.users ";
...
//                .ok();
        String s1="select  "
                          ...
                          + ",p.user_name"
                          + ",p.password"
                          + ",p.user_level from db_bulletin.users p where user_name='"+user_name+"'" ;
        try {
            Connection conn = PoolConnection.connect();
    ...
    public static List<String> get_cashiers(){
        ...
        try {
            Connection conn = PoolConnection.connect();
            String s0="select user_name from db_bulletin.users where user_level=0";
            Statement stmt = conn.createStatement();
            ResultSet rs = stmt.executeQuery(s0);
            while (rs.next()) {
